...
function getSqlDataToDomTree(){
	...
	db_baicaowei.transaction(function (tx) {
	   tx.executeSql('SELECT * FROM goodsList2', [], function (tx, results) {
	      ...
	      console.log(len);
		  for(var j=0;j<len;j++){
	     		 console.log(results.rows.item(j).goodsImgSrc);
...
//				   		tx.executeSql('SELECT * FROM shopCar',[],function(tx, results2){
//				    			var len2 = results2.rows.length, i;
//				    			var lastShopId = results2.rows.item(len-1).shopCarId;
//				    			// 如果购物车不存在该商品则记录+1.
//				    			addOneShopRecord(lastShopId);
//					    },null);
					});
				});
			// 如果购物车存在该商品则数量+1.
